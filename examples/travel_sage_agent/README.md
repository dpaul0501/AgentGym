...
end, against a running agent rather than a synthetic test fixture:

- **`MEMORY` (context lever)** — a mock flight-search tool returns ~30 verbose results, causing
  context bloat. `HeadroomMemoryOptimizer` searches `headroom-ai` compression configs and the
  winning one is applied live, in the loop.
- **`GUARDRAILS` (harness lever)** — a mock hotel-search tool's review field carries a prompt-
  injection payload. `GuardrailsOptimizer` runs a `garak` probe against the agent's backing model
  and produces a tightened `invariant-ai` policy, enforced live: the malicious tool output is
  stripped before it ever reaches the model's context — not dependent on whether the model happens
...
**`demo_context_lever.py`**: runs the flight-search task against Bedrock Claude with no `MEMORY`
artifact bound (raw tool output reaches the model — char count printed); captures the trace into a
`TrainingCorpus`; runs `HeadroomMemoryOptimizer.estimate()` (an Optuna search over
`headroom.CompressConfig` variants); prints the winning config and its measured
`tokens_before`/`tokens_after`/`tokens_saved`; re-runs the same task with that config bound live,
to confirm the agent still answers correctly with the compressed context.
...
the model. Runs `GuardrailsOptimizer.estimate()`: a `garak` probe against the Ollama target,
producing an `invariant-ai` policy. Re-runs the same task with that policy bound — the injected
review is now blocked at the tool boundary (`{"blocked": true, ...}` in place of the raw content)
...
the injection payload in this repo, unprompted, with no guardrail bound at all — and local
`qwen2.5:7b` didn't comply with it either. Rather than hand-craft a payload specifically to defeat
...
remove the exposure deterministically, independent of model judgment.
...
- `benchmark.py` — the two `Task` prompts both demos use.
